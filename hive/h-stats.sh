...
stats_raw=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstat1"}' | nc -w 10 localhost 4060 | jq '.result'`
...
#	local stats_detail=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstatdetail"}' | nc -w $API_TIMEOUT localhost $CUSTOM_API_PORT | jq '.result'`
	local stats_detail=`echo '{"id":0,"jsonrpc":"2.0","method":"miner_getstatdetail"}' | nc -w 10 localhost 4060 | jq '.result'`
	...
	if [[ -z $bus_numbers || $bus_numbers == "[]" ]]; then
		stats=$(jq -n \
			...
			--arg ver "$ver" \
			'{$hs, $temp, $fan, $uptime, ar: [$ac, $rj, $iv], $ver}')
	else
		stats=$(jq -n \
			--arg uptime "`echo \"$stats_raw\" | jq -r '.[1]' | awk '{print $1*60}'`" \
			--argjson hs "$hs" --argjson temp "$temp" --argjson fan "$fan" \
			...
			'{$hs, $temp, $fan, $uptime, ar: [$ac, $rj, $iv, $iv_bus], $bus_numbers, $ver}')
	fi
fi
